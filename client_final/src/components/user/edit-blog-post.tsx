...
const EditBlogPost = () => {
  ...
  const fetchData = async () => {
    API.get(`/user/get-single-blog-data/${editBlogId}`).then((res) => {
      setTitle(res.data.post.title);
      if (editor) {
        editor.commands.setContent(res.data.post.content); // Set editor content with fetched data
      }
    });
  };
  ...
  const handleEdit = async (e: { preventDefault: () => void; }) => {
    ...
    e.preventDefault();
    API.post(`/upload/blogdata/edit/${editBlogId}`, {
      user,
      title,
      content: editor.getHTML()
    }).then((res) => {
      alert(res.data.message)
      navigate("/user-dashboard/get-blog-posts")
    });
  };
